...
def print_death_age(people_dict: dict) -> None:
    ...
    for i in people_dict:
        name = people_dict[i][NAME_INDEX]
        birth_date = people_dict[i][BIRTH_YEAR_INDEX]
        death_date = people_dict[i][DEATH_YEAR_INDEX]
        age = death_date - birth_date
        print(f"{name}, died at the age of {age}")
    return None


def count_genders(people_dict: dict) -> None:
    ...
    f = 0
    for i in people_dict:
        gender = people_dict[i][GENDER_INDEX]
        match gender:
            case "F":
                m += 1
            case "M":
                f += 1
            case _:
                print(f"ERROR! Invalid gender option at record: {i}")
                return None
    print(f"""Males:   {m}
Females: {f}""")
    return None
...
def print_marriages(marriages_dict: dict, people_dict: dict) -> None:
    ...
    for marriage_key in marriages_dict:
        ...
        h_age = m_date - husband[BIRTH_YEAR_INDEX]
        w_age = m_date - wife[BIRTH_YEAR_INDEX]
        
        print(f"{h_name} married in {m_date} at the age of {h_age} with {w_name}, who was {w_age}")
